...
public class TG_Strategy_Automatic_new extends Study
{
    ...
    private static final String ADDITIONAL_ORDER_TICKS = "ADDITIONAL_ORDER_TICKS";
    ...
    private int stopTicks = 50, profitTicks = 20, additionalOrderTicks = 10, entryTicks = 4;
    ...
    public void onBarClose(OrderContext ctx){
      ...
      /*  for(TimeFrame tf : getSettings().getSessions()) if (tf.isEnabled()) {
            if ((
                    (ctx.getCurrentTime() >= tf.getStartTime()+mn && ctx.getCurrentTime() <= tf.getEndTime()+mn)
                            &&
                            !(currentTime>= tf.getStartTime()+mn && currentTime <= tf.getEndTime()+mn)
            ) ||
                    (
                            (currentTime >= tf.getStartTime()+mn && currentTime <= tf.getEndTime()+mn)
                                    &&
